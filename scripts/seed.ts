...
async function seed() {
  try {
    console.log("Connecting to the Atelier database...");
    await dbConnect();

    console.log("Purging existing inventory records...");
    await Product.deleteMany({});
    await Category.deleteMany({});

    // 1. Create Categories
    console.log("Establishing product categories...");
    ...
    console.log("Curating 10 premium coffee selections...");
    const products = [
      {
        name: "Midnight Noir",
    ...
    console.log("✅ Atelier Inventory Seeded Successfully.");
    process.exit(0);
  } catch (error) {
